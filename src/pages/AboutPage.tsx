  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] flex flex-col">
      ...
      <div className="flex-1 w-full max-w-[1400px] mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-5 items-start">
        <div className="lg:hidden"><ServiceMenuChips active="about" /></div>
          ...
          <div className="card">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 pb-5 border-b border-gray-100">
              ...
              <div className="text-center sm:text-left">
                <h2 className="text-4xl font-extrabold text-navy-700 leading-tight">{ORG_INFO.nameTh}</h2>
                <p className="text-xl text-gray-500 mt-1">{ORG_INFO.nameEn}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 pt-5">
              ...
              <ContactRow icon={Mail} label="อีเมล (สารบรรณกลาง)">
                <a href={`mailto:${ORG_INFO.email}`} className="hover:underline">{ORG_INFO.email}</a>
              </ContactRow>

              <ContactRow icon={Clock} label="เวลาทำการ">
                <p>{ORG_INFO.officeHours}</p>
              </ContactRow>
            </div>

            <div className="pt-6">
              <a
                href={ORG_INFO.mapsUrl}
                target="_blank"

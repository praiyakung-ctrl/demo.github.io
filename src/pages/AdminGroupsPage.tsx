...
const EMPTY_PERMISSIONS: Record<MenuKey, PermissionAction[]> = {
  map: [], dashboard: [], portal: [], reports: [], comparisonReport: [], lprSearch: [], policeUsage: [],
  ...
  adminMemberReview: [], adminCctvApprovers: [],
};
...
export function AdminGroupsPage() {
  ...
  const pageRows = groups.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="bg-blue-50 border-b-2 border-blue-100 px-6 py-4 flex items-center justify-between shadow-sm flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-navy-700">จัดการกลุ่มและสิทธิ์</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-sm text-gray-600">กลุ่มผู้ใช้งานทั้งหมด</span>
                <span className="bg-navy-700 text-white text-sm font-bold px-2.5 py-0.5 rounded-full">{groups.length} กลุ่ม</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="card overflow-hidden shadow-md">
            <div className="overflow-x-auto">
              <table className="w-full text-xl">
                <thead className="bg-blue-200">
                  <tr>
                    <th scope="col" className="text-left text-xl font-semibold text-navy-700 px-4 py-2.5">ชื่อกลุ่ม</th>
                    <th scope="col" className="text-left text-xl font-semibold text-navy-700 px-4 py-2.5">คำอธิบาย</th>
                    <th scope="col" className="text-center text-xl font-semibold text-navy-700 px-4 py-2.5">เมนูที่เข้าถึงได้</th>
                    <th scope="col" className="text-center text-xl font-semibold text-navy-700 px-4 py-2.5">ประเภท</th>
                    <th scope="col" className="text-center text-xl font-semibold text-navy-700 px-4 py-2.5">ดำเนินการ</th>
                  </tr>
                </thead>
                <tbody>
                  {pageRows.map(group => (
            ...
            <Pagination total={groups.length} page={safePage} pageSize={PAGE_SIZE} onPageChange={setPage} />
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="rounded-xl border border-gray-200 overflow-hidden">
              <table className="w-full text-lg">
                <thead className="bg-blue-200">
                  <tr>
                    <th scope="col" className="text-left text-xl font-bold text-navy-700 px-4 py-2.5">เมนู</th>
                    {ACTION_OPTIONS.map(a => (
                      <th key={a.key} scope="col" className="text-center text-xl font-bold text-navy-700 px-3 py-2.5 w-20">{a.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
